...
from utils_py.util import AnglesTraces
...
class TapasPlayer(object):
    ...
    def __init__(self, controller, view_controller, parser, media_engine, csv_file, 
        ...
        self.anglesTraces = AnglesTraces(csv_file)
...
########################### VIEW METHODS #####################################
    ...
    def setCurrentAngles(self):
        '''
        Reads the current angles (alpha, beta and gammma) from the csv file 
        
        '''        
        #- self.t_experiment_started
        self.cur_angles = self.anglesTraces.getAngles(time.time() - self.t_experiment_started)        
